<template>
  <div>
    ...
    <el-dialog title="查询模板修改" :visible.sync="templateDialog" width="70%" :close-on-click-modal="false">
      <el-form size="small" :inline="true">
      ...
      <el-tabs v-model="templateTab">
          ...
          <el-row :gutter="20" v-for="it in templateQueryContent" class="dynamicLine" :key="it.key">
            ...
            <el-col :span="4">
              <el-select v-model="it.queryConditionValue" clearable placeholder="请选择" size="small">
                <el-option
                  v-for="item in templateQueryCondition"
                  :key="item.value"
                  :label="item.label"
                  clearable
                  :value="item.value">
        ...
        <el-tab-pane label="表格展示" name="third">
          <el-row>
            <el-col :span="4">
              <div>
...
export default {
  ...
  name: 'dynamic-base',
  props: ['templateConfig', 'page', 'queryType', 'tableShow', 'tableRef', 'templateQueryCondition'],
  created() {
    this.listTemplate()
  ...
  methods: {
    ...
    refreshTable() {
      if (this.tableRef) {
        const originColumns = this.tableRef.store.states._columns
        if (!this.backupTable) {
        ...
        this.tableRef.store.states._columns = prefix.concat(newRemaining).concat(suffix)
        this.tableRef.store.updateColumns()
      }
    },
    copyTemplate() {
      if (this.templateId) {
